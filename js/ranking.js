...
async function buildRanking() {
  ...
  const rows  = await Promise.all(users.map(async user => {
    ...
    const {filled:kF,total:kT} = countKnockoutFilled(scores,picks);
    return { id:user.id, name:user.name, total:scored.total, groupPts:scored.groupPts, koPts:scored.koPts, maxPts, filled:gF+kF, totalSlots:gT+kT, champion:scored.champPick };
  }));
...
async function renderAdminRanking() {
  ...
  container.innerHTML = `
    ...
    <div id="copy-feedback" style="display:none;background:#dcfce7;color:#15803d;border:1px solid #86efac;border-radius:8px;padding:8px 14px;font-size:13px;font-weight:600;margin-bottom:12px;text-align:center">
      ✅ Ranking copiado!
    </div>
    <div class="ranking-table-wrap">
      <table class="ranking-table">
